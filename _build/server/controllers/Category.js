...
var Category = {
  ...
  createCategory: function createCategory(request, response, next) {
    ...
    pool.query('SELECT * FROM categories WHERE category_name= $1', [categoryName], function (err, res) {
      if (err) return next(err);
      if (res.rowCount !== 0) {
      ...
      pool.query('INSERT INTO categories(category_name) VALUES($1)', [categoryName], function (err, res) {
        if (err) return next(err);
